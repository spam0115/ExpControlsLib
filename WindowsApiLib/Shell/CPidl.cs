...
{
    /// <summary>
    /// Base class for PIDL helpers. Contains only static methods that operate on
    /// native (IntPtr) PIDLs and byte[] PIDL structures. This class does NOT hold
    /// a local managed copy of a PIDL's bytes. For a managed copy, use <see cref="CPidlLocal"/>.
    /// </summary>
    /// <remarks>
    /// Implements <see cref="IDisposable"/> and a finalizer so that any unmanaged PIDL
    /// <see cref="IntPtr"/> this instance takes ownership of (via <see cref="TakeOwnershipOf"/>,
    /// e.g. the result of <c>ILCreateFromPathW</c>) is safely released through the COM
    /// allocator (<see cref="Marshal.FreeCoTaskMem"/>) when the instance is disposed or
    /// garbage-collected. Descendants that allocate native PIDLs should assign them here.
    /// </remarks>
    public abstract class CPidl : IEnumerable, ICPidl, IDisposable
    {
        #region        Unmanaged resource management

        // Unmanaged PIDL this instance owns (allocated via CoTaskMem by the shell, e.g.
        // ILCreateFromPathW / ILClone / ILCombine). Released on Dispose or finalization.
        private IntPtr m_ownedPidl = IntPtr.Zero;
        private bool m_disposed;

        /// <summary>
        /// The unmanaged PIDL <see cref="IntPtr"/> this instance owns and will release
        /// on dispose/finalize. <see cref="IntPtr.Zero"/> when this instance owns no
        /// native PIDL (e.g. when constructed from a caller-owned <see cref="IntPtr"/>).
        /// </summary>
        public IntPtr OwnedPidl => m_ownedPidl;
        ...
        /// <summary>
        /// Takes ownership of <paramref name="pidl"/>. The pointer must have been
        /// allocated with the COM allocator (e.g. by <c>ILCreateFromPathW</c>,
        /// <c>ILClone</c>, <c>ILCombine</c>). It will be freed via
        /// <see cref="Marshal.FreeCoTaskMem"/> when this instance is disposed or
        /// garbage-collected. Any previously owned PIDL is released first.
        /// </summary>
        /// <param name="pidl">The native PIDL to take ownership of. May be <see cref="IntPtr.Zero"/>.</param>
        protected void TakeOwnershipOf(IntPtr pidl)
        {
            if (m_disposed)
                throw new ObjectDisposedException(nameof(CPidl), "Cannot take ownership of a PIDL on a disposed CPidl.");

            // Release any previously owned PIDL before overwriting.
            if (m_ownedPidl != IntPtr.Zero && m_ownedPidl != pidl)
            {
                Marshal.FreeCoTaskMem(m_ownedPidl);
            }
            m_ownedPidl = pidl;
        }

        /// <summary>
        /// Releases the owned native PIDL and suppresses finalization.
        /// </summary>
        public void Dispose()
        {
        ...
        /// <summary>
        /// Releases the owned native PIDL. <paramref name="disposing"/> is true when
        /// called explicitly via <see cref="Dispose()"/>; false when called from the
        /// finalizer. The unmanaged PIDL is released in both cases.
        /// </summary>
        /// <param name="disposing">True if called from <see cref="Dispose()"/>; false if from the finalizer.</param>
        protected virtual void Dispose(bool disposing)
        {
            if (m_disposed) return;

            // Free unmanaged PIDL regardless of disposing flag — it is unmanaged.
            if (m_ownedPidl != IntPtr.Zero)
            {
                Marshal.FreeCoTaskMem(m_ownedPidl);
                m_ownedPidl = IntPtr.Zero;
            }
        ...
        /// <summary>
        /// Finalizer. Releases the owned native PIDL if <see cref="Dispose()"/> was not called.
        /// </summary>
        ~CPidl()
        {
        ...
        /// Implemented by <see cref="CPidlLocal"/>.
        /// </summary>
        public abstract byte[] PidlBytes { get; }
        ...
        /// <remarks>Performs a direct byte comparison and does NOT take ownership of either
        /// native PIDL, so callers retain ownership of both inputs.</remarks>
        public static bool StartsWith(IntPtr pidl1, IntPtr pidl2)
        {
            int cb1 = GetPidlLength(pidl1);
            int cb2 = GetPidlLength(pidl2);
            if (cb2 > cb1)
                return false; // B is longer than A
            for (int i = 0; i < cb2; i++)
            {
                if (Marshal.ReadByte(pidl1, i) != Marshal.ReadByte(pidl2, i))
                    return false;
            }
            return true;
        }
        ...
        public abstract bool IsBinaryEqual(CPidl other);
        ...
        */
        ...
        /// <summary>
        /// Copy the contents of a byte() containing a PIDL to CoTaskMemory,
        /// returning an IntPtr that points to that mem block.
        /// Implemented by <see cref="CPidlLocal"/>.
        /// </summary>
        /// <remarks> Caller must Free the returned IntPtr when done with the returned PIDL.</remarks>
        public abstract IntPtr ToPIDL();
        ...
        /// Implemented by <see cref="CPidlLocal"/>.
        /// </summary>
        public abstract object[] Decompose();
        ...
        /// beginning of this instance of CPidl.
        /// Implemented by <see cref="CPidlLocal"/>.</summary>
        public abstract bool StartsWith(CPidl cp);
        ...
        /// <summary>
        /// Returns the IShellFolder for this cPidl's bytes.
        /// Implemented by <see cref="CPidlLocal"/>.
        /// </summary>
        public abstract IShellFolder GetIShellFolder();
