...
namespace WindowsApiLib.Shell
{
    ...
    public interface IStream
    {
        ...
        [PreserveSig]
        int LockRegion(long libOffset, long cb, LOCKTYPE dwLockType);

        // Removes the access restriction on a range of bytes previously restricted 
        // with IStream::LockRegion
        [PreserveSig]
        int UnlockRegion(long libOffset, long cb, LOCKTYPE dwLockType);

        // Retrieves the STATSTG structure for this stream
        [PreserveSig]
        int Stat(out ShellAPI.STATSTG pstatstg, STATFLAG grfStatFlag);
